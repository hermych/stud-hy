...
class Universidad
{
  ...
  {
    $sql_univ = "SELECT id_universidad, u.nombre, descripcion, imagen_url as 'imagen', d.nombre as 'departamento', u.id_departamento, u.id_distrito, u.id_provincia, p.nombre as 'provincia', di.nombre as 'distrito' FROM universidades as u, departamentos as d, provincias as p, distritos as di WHERE u.id_departamento = d.id_departamento AND u.id_provincia = p.id_provincia AND u.id_distrito = di.id_distrito;";
  ...
  public function universidadGDelete($iduniv)
  {
    ...
    if (count($busqueda) == 0) {
      $consulta = "DELETE FROM `universidades` WHERE `universidades`.`id_universidad` = $iduniv";
